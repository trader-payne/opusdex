...
OpusDex is a bash-based orchestrator that coordinates Claude Code CLI, OpenAI Codex CLI, and optionally Cursor agent CLI through a structured development workflow.
...
3. **Review & Live Validation** (Claude + Cursor) — Code review with verdict (APPROVE/REQUEST_CHANGES/BLOCK). Uses `--resume` to continue the plan conversation.
   - If REQUEST_CHANGES/BLOCK → **Fix & Retest** (Codex, single session) — fix + retest in one call → re-review
   - **Live Pass** (Cursor, optional) — Validates the app in a live environment. If it fails, Cursor can retry fixes. If Cursor exhausts retries, it outputs `live_feedback.md`, and Claude re-reviews to design a fix. If Cursor successfully fixes issues, Codex runs a **Post-Live Verify** test pass before the loop continues.
...
- **Cursor**: live = test + fix via `agent --print --model ... --output-format text` (plus `--yolo`, `--trust`, `--approve-mcps`).
...
- YOLO mode is always on: Claude uses `--dangerously-skip-permissions`, Codex and Cursor use `--yolo`.
